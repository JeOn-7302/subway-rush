...
M_CH_FOIL_D = new_mat("ITM_ChocoFoilShade", "8E969D", 0.30)
M_CH_SHD = new_mat("ITM_ChocoShadow", "2E1810", 0.60)     # 포일이 지는 그늘  # 은박 접힘면
...
# 갈색 구역이 넓으면 그쪽이 무거워 보여 막대가 한쪽으로 기운 공구처럼 된다.
YG_DARK_W = YG_L * 0.12              # 한쪽 끝 12% 만 짙은 갈색
_YGF = -YG_D / 2.0
...
_p.append(decal("yg_panel", _rr(YG_L * 0.34, YG_H * 0.60, 0.0018, 2,
                                cx=YG_L * 0.08),
...
BM_RINGS = [(0.0000, 0.01310), (0.0019, 0.01496), (0.0051, 0.01616),
            (0.0109, 0.01657),
            (0.0217, 0.01714), (0.0345, 0.01733), (0.0405, 0.01699),
            (0.0471, 0.01615), (0.0543, 0.01454), (0.0609, 0.01257),
            (0.0664, 0.01019), (0.0697, 0.00853)]
...
_p.append(revolve("bm_cap", [(0.0629, 0.01079), (0.0680, 0.01110),
                             (0.0695, 0.01148),
                             (0.0761, 0.01133), (0.0771, 0.01034)],
                  BM_SEG, M_BM_CAP))
_BMP = body_place(BM_RINGS, BM_SEG)
# 바나나 — 초록 라벨 아래, 턱 바로 위. 인쇄 그래픽이라 돌출 0.20mm.
_BAN_R, _BAN_CZ, _BAN_A, _BAN_T = 0.0056, 0.0432, 1.32, -30.0
_blo, _bup = _edges(
...
# --------------------------------------- 초콜릿 · 은박을 옆으로 젖힌 판
# 뜯긴 자리를 판의 **짧은 끝**에 두고 세로로 세웠더니, 포일이 아래에 깔려
# 몇 번을 얇게 해도 **회색 홀더 위에 초콜릿을 꽂아 놓은 것**으로 읽혔다.
# 서 있는 물건 밑에 놓인 밝은 덩어리는 무엇이든 받침이 된다 — 색·두께·
# 뜯긴 모양을 손대는 걸로는 못 고친다. 자리를 옮겨야 고쳐진다.
#
# 그래서 **좁은 옆면 쪽에서** 젖힌다. 포일이 왼쪽 세로 띠를 덮고 오른쪽
# 초콜릿이 드러나면, 바닥에 닿는 것이 아무것도 없어 받침이 생기지 않는다.
# 실제로도 긴 바는 옆을 갈라 밀어 내며 먹는다.
CH_W, CH_H, CH_D = 0.097, 0.043, 0.0085
CH_COL, CH_ROW = 3, 2
CH_SIDE = -0.0110          # 포일이 끝나는 z. 한쪽 24% 만 덮는다
_p = [plate("ch_bar", CH_W, CH_H, 0.0022, CH_D, M_CH, seg=2,
            edge_mat=M_CH_LIT)]
bevel(_p[0], 0.0007, 1)
# 조각 여섯 — 판 위로 0.7mm 솟은 칸. 사이 간격이 그대로 **쪼개는 홈**이 된다.
...
for _r in range(CH_ROW):
    for _c in range(CH_COL):
        ...
        _b = plate("ch_blk%d%d" % (_r, _c), _CW, _CHH, 0.0014, CH_D + 0.0006,
                   M_CH, seg=1, cx=_cx, cz=_cz, edge_mat=M_CH_LIT)
        bevel(_b, 0.0005, 1)         # 약한 베벨 — 각지면 장난감 블록이다
        _p.append(_b)
# 포일 두께는 **판 두께에 딸린 예산**이다. 조각이 솟은 높이보다 포일이
# 두꺼워야 덮인다. 조각 0.6mm(한쪽 0.3) · 포일 한쪽 0.4mm — 판(8.5mm)의 4.7%.
# 왼쪽 조각들은 지우지 않는다. 포일이 그 위를 지나며 덮으므로, 경계에
# 걸친 칸은 절반만 보인다 — 젖혀 놓은 포장은 원래 그렇게 보인다.
CH_FD = CH_D + 0.0008
_FH = CH_H / 2.0 + CH_SIDE                   # 덮는 폭 12.0mm
_FCZ = (-CH_H / 2.0 + CH_SIDE) / 2.0
# 판보다 **좁게** 만든다. 0.6mm 내밀었더니 위아래 끝으로 은색이 삐져나와
# 판보다 긴 은색 막대가 하나 더 붙은 것처럼 보였다.
_p.append(plate("ch_foil", CH_W - 0.0016, _FH, 0.0012, CH_FD, M_CH_FOIL,
                seg=2, cz=_FCZ, edge_mat=M_CH_FOIL_D))
bevel(_p[-1], 0.0005, 1)
# 젖힌 경계 — 어긋나게 물린다. 곧은 세로선이면 초콜릿 판과 은색 판을
# 나란히 붙여 놓은 것으로 보인다(실측: 문짝 두 장처럼 읽혔다).
_TW = CH_W / 2.0
for _i, (_t0, _t1, _td) in enumerate(((-0.98, -0.45, 0.0036),
                                      (-0.45, 0.22, 0.0010),
                                      (0.22, 0.98, 0.0028))):
    _p.append(plate("ch_tear%d" % _i, (_t1 - _t0) * _TW, _td, 0.0004, CH_FD,
                    M_CH_FOIL, seg=1, cx=(_t0 + _t1) / 2.0 * _TW,
                    cz=CH_SIDE + _td / 2.0 - 0.0004, edge_mat=M_CH_FOIL_D))
# **격자를 포일 위로 이어 긋는다.** 이게 이 물건의 핵심 단서다. 포일 면이
# 민무늬면 초콜릿 옆에 놓인 별개의 은색 판이 되고, 경계를 아무리 찢어 놔도
# 그 해석이 안 깨진다(지그재그로 물려 봤더니 이번엔 퍼즐 조각이 됐다).
# 조각을 가르는 홈이 포일 밑에서도 그대로 이어져 눌려 보이면, 두 면이
# **같은 판의 덮인 쪽과 드러난 쪽**으로 묶인다.
for _c in range(CH_COL - 1):
    _gx = -CH_W / 2.0 + 0.0030 + _CW + 0.0011 + (_CW + 0.0022) * _c
    _p.append(plate("ch_press%d" % _c, 0.0022, _FH - 0.0016, 0.0005,
                    CH_FD + 0.0004, M_CH_FOIL_D, seg=1, cx=_gx, cz=_FCZ))
# 그늘 한 줄 — 경계 **초콜릿 쪽**에 어두운 띠를 둔다. 포일이 위에 얹혀
# 그림자를 지운다는 뜻이라, 두 면의 앞뒤가 정해진다. 이게 없으면 같은
# 평면에 나란히 놓인 두 판으로만 보인다.
_p.append(plate("ch_shadow", CH_W - 0.0030, 0.0016, 0.0004, CH_D + 0.0009,
                M_CH_SHD, seg=1, cz=CH_SIDE + 0.0011))
# 말린 가장자리 — 젖힌 포일은 끝이 **두 겹**이 된다. 그 도톰한 선 하나가
# '찢어진 종이'와 '덧대어 놓은 판'을 가르는 가장 짧은 단서다.
for _i, (_r0, _r1) in enumerate(((-0.98, -0.44), (-0.44, 0.24), (0.24, 0.98))):
    _rd = (0.0032, 0.0009, 0.0026)[_i]
    _p.append(plate("ch_roll%d" % _i, (_r1 - _r0) * _TW - 0.0008, 0.0017,
                    0.0007, CH_FD + 0.0009, M_CH_FOIL, seg=1,
                    cx=(_r0 + _r1) / 2.0 * _TW,
                    cz=CH_SIDE + _rd - 0.0012, edge_mat=M_CH_FOIL_D))
for _o in _p:
    shade(_o, False)
_ch = finish("ITM12_Chocolate", _p)
# 세로로 세운다 — 긴 축을 Z 로 돌린다. 판이 97×43 이라 세우면 2.26:1,
# 눕혔을 때보다 '세로로 긴 얇은 판'이 확실해진다. 젖힌 포일은 왼쪽 세로 띠로
# 가고, 바닥에는 초콜릿만 닿는다.
...
SD_R = 0.01451         # 지름 29.3 → 29.0mm
# 상하 금속 림은 **좁혔다** — 아래 6.3 → 5.0mm, 위 3.9 → 3.3mm.
SD_RINGS = [(0.0000, 0.01128), (0.0025, 0.01402), (0.0053, SD_R),
            (0.0804, SD_R), (0.0898, 0.01316), (0.0954, 0.01085),
            (0.0972, 0.01126), (0.0989, 0.01163)]
...
_p.append(revolve("sd_lid", [(0.0989, 0.01136), (0.0997, 0.01079)],
                  SD_SEG, M_SD_GUN))
_p.append(slab("sd_score", (-0.0080, 0.0, 0.09952), (0.0085, 0.0097, 0.0008),
               M_SD_GUN))
_p.append(slab("sd_tab", (0.0032, 0.0, 0.10000), (0.0133, 0.0049, 0.0013),
               M_SD_MET))
_p.append(slab("sd_tab_hole", (0.0057, 0.0, 0.10073), (0.0061, 0.0024, 0.0010),
               M_SD_GUN))
_p.append(slab("sd_rivet", (-0.0034, 0.0, 0.10061), (0.0040, 0.0040, 0.0015),
               M_SD_MET))
...
_rlo, _rup = _edges(lambda t: (t * _RB, 0.0457 + t * 0.0301),
...
for _i, (_bx, _bz, _bd) in enumerate((
        (-0.0040, 0.0717, 0.0034), (0.0026, 0.0775, 0.0026),
        (0.0072, 0.0684, 0.0021))):
...
# 정면 실루엣(70×98)은 건드리지 않고 **두께만** 31.5 → 27.3mm (−13%).
# 앞뒤가 같은 필로우라 후면에도 같은 볼륨이 남는다 — 뒤가 평평한 판이면
# 봉지가 아니라 상자다.
SN_W, SN_H, SN_D = 0.070, 0.098, 0.0273
...
for _s in (-1, 1):
    _p.append(plate("sn_seal%d" % (_s > 0), SN_W * 0.92, 0.0072, 0.0010, 0.0012,
